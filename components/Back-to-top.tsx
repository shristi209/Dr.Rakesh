...
export default function BackToTop() {
  ...
  const checkBackground = useCallback(() => {
    ...
    if (footer) {
      ...
      const buttonRect = document.getElementById('back-to-top')?.getBoundingClientRect()
  ...
  return (
    <button
      ...
      className={`fixed bottom-6 right-6 p-2 rounded-full shadow-lg transition-all duration-300 z-50 backdrop-blur-sm
        ${isLight ? 'bg-white/70 text-[#1b3a2d] hover:bg-white' : 'bg-[#1b3a2d]/70 text-white hover:bg-[#1b3a2d]'}
